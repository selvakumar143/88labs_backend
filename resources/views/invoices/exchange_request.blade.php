...
<body>
    <div class="header">
        <p class="title">Exchange Request Invoice</p>
    ...
    <table class="meta">
        ...
        <tr><td>Client Email</td><td>{{ $invoice['client']['email'] ?? '-' }}</td><td>Approved By</td><td>{{ $invoice['approved_by'] ?? '-' }}</td></tr>
        <tr><td>From Currency</td><td>{{ $invoice['details']['base_currency'] ?? '-' }}</td><td>To Currency</td><td>{{ $invoice['details']['conversion_currency'] ?? '-' }}</td></tr>
        <tr><td>Rate</td><td>{{ $invoice['details']['conversion_rate'] ?? '-' }}</td><td>Return Amount</td><td>{{ number_format((float) ($invoice['details']['return_amount'] ?? 0), 2) }}</td></tr>
    ...
    <div class="section right">
        ...
        <div><strong>Total Deduction: {{ number_format((float) $invoice['total'], 2) }} {{ $invoice['currency'] ?? '' }}</strong></div>
    </div>
</body>
</html>
